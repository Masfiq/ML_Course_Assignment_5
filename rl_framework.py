...
class Agent(ABC):
    ...
    def update_Qn(self):
        ...
        if len(last_steps) == 0:
            # not episodic. set last sample to be done state
            self.Done[-1] = True
            last_steps = np.where(self.Done)[0]

        # print('last_steps', last_steps, len(self.Done))
        first = 0
    ...
    def train(self, n_epochs, method, learning_rate, gamma, verbose=True):
        ...
        self.Qnet.train(self.X, T, self.X, T,
                        n_epochs=n_epochs, method=method, learning_rate=learning_rate, verbose=verbose)
